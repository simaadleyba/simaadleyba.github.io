...
function computeRadialLayout(nodes, width, height) {
  ...
  const byDepth = {};
  nodes.forEach(n => { if (!byDepth[n.depth]) byDepth[n.depth] = []; byDepth[n.depth].push(n.id); });
  ...
  for (let depth = 1; depth <= maxDepth; depth++) {
    ...
    nodesAtDepth.forEach(id => {
      ...
      if (!siblingGroups[node.parent]) siblingGroups[node.parent] = [];
      siblingGroups[node.parent].push(id);
    });
    Object.entries(siblingGroups).forEach(([parentId, siblings]) => {
      const pRange = angleRange[+parentId] || { start: 0, end: 2 * Math.PI };
      const sliceSize = (pRange.end - pRange.start) / siblings.length;
      siblings.forEach((id, i) => {
        const angle = pRange.start + sliceSize * (i + 0.5);
        positions[id] = { x: cx + r * Math.cos(angle - Math.PI / 2), y: cy + r * Math.sin(angle - Math.PI / 2) };
        angleRange[id] = { start: pRange.start + sliceSize * i, end: pRange.start + sliceSize * (i + 1) };
      });
    });
  }
  return positions;
...
export default function BetheLattice() {
  ...
  const positions = useMemo(() => computeRadialLayout(nodes, 400, 360), [nodes]);

  const levelCounts = useMemo(() => {
    const counts = [1];
    for (let l = 1; l <= d; l++) counts.push(l === 1 ? k : k * Math.pow(k - 1, l - 1));
    return counts;
  }, [k, d]);

  const totalNodes = nodes.length;
  const theoreticalTotal = k === 2 ? 1 + 2 * d : Math.round(1 + (k * (Math.pow(k - 1, d) - 1)) / (k - 2));
    ...
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', marginBottom: '1rem' }}>
        <div style={{ flex: '1 1 160px' }}>
          <label style={{ fontSize: '0.8rem', color: 'var(--muted)', display: 'block', marginBottom: '0.3rem' }}>k = {k} (coordination number)</label>
          <input type="range" min={2} max={6} value={k} onChange={e => setK(+e.target.value)} style={{ width: '100%', accentColor: 'var(--accent)' }} />
        </div>
        <div style={{ flex: '1 1 160px' }}>
          <label style={{ fontSize: '0.8rem', color: 'var(--muted)', display: 'block', marginBottom: '0.3rem' }}>d = {d} (depth)</label>
          <input type="range" min={1} max={6} value={d} onChange={e => setD(+e.target.value)} style={{ width: '100%', accentColor: 'var(--accent)' }} />
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        <div style={{ flex: '1 1 280px', minWidth: 0 }}>
          <svg viewBox="0 0 400 360" style={{ width: '100%', background: 'var(--accent-bg)', borderRadius: '8px' }}>
            ...
            {nodes.map(({ id, depth }) => {
              ...
              if (!pos) return null;
              return <circle key={id} cx={pos.x} cy={pos.y} r={Math.max(3, 9 - depth * 1.0)}
        ...
        <div style={{ flex: '1 1 180px', minWidth: 0 }}>
          <div style={{ background: 'var(--accent-bg)', borderRadius: '8px', padding: '0.8rem', fontSize: '0.82rem', border: '1px solid var(--border)' }}>
            <div style={{ color: 'var(--muted)', marginBottom: '0.6rem', fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              Node Counts by Level
            </div>
            {levelCounts.map((count, level) => (
              <div key={level} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.35rem' }}>
                <div style={{ width: 10, height: 10, borderRadius: '50%', background: DEPTH_COLORS[level % DEPTH_COLORS.length], flexShrink: 0 }} />
                <span style={{ color: 'var(--muted)' }}>Level {level}:</span>
                <span style={{ color: 'var(--text)', fontFamily: 'monospace' }}>{count}</span>
              </div>
            ))}
          ...
          <div style={{ marginTop: '0.6rem', display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
            {Array.from({ length: d + 1 }, (_, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', fontSize: '0.75rem' }}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: DEPTH_COLORS[i % DEPTH_COLORS.length] }} />
                <span style={{ color: 'var(--muted)' }}>d={i}</span>
              </div>
            ))}
          </div>
        </div>
